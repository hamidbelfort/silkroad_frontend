...
export default function SettingsPage() {
  ...
  const emailSchema = z.object({
    smtpUser: requiredMinString(
      3,
      "validation.settings.smtpUser"
    ),
    smtpPass: requiredMinString(
      3,
      "validation.settings.smtpPassword"
    ),
    adminEmail: z
      .string()
      .email(t("validation.settings.email")),
  ...
  const onSubmitGeneral = async (data: GeneralForm) => {
    ...
    if (res && res.success)
      toast.success(t("message.settings.success"));
